...
def fix_length_byinter(hori, vert):
    ...
    def _shorten(lines):
        for i, inter in enumerate(inters):
            x0, y0, x1, y1 = np.ravel([inter[0], inter[-1]])
            new = np.array([x0, y0, x1, y1, 0, lines[i, 5]], dtype='int32')
...
def remove_fake_outer(lines, ll, kind):
    ...
    def _remove_fake_outer(lines, med, where):
        ...
        elif dkind < 15 and dother > 5 and length(line0) < med*0.8:
            lines = np.delete(lines, where, axis=0)
        elif dkind < 30 and dother > 5 and length(line0) < med*0.7:
            lines = np.delete(lines, where, axis=0)
        return lines
